...
| Login POST hangs on spinner indefinitely | PayPal flagged the session — try a fresh residential proxy and delete `sessions/pp_profile/` |
| `Der Zugriff ist vorübergehend eingeschränkt` interstitial | Proxy IP is on PayPal's datacenter blacklist — use a different residential proxy |
| `Please wait while we perform security check` hangs forever | Arkose/risk engine flagged the session — try another residential proxy + fresh profile |
